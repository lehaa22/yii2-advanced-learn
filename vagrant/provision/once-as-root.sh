...
apt-get install -y php7.1-curl php7.1-cli php7.1-intl php7.1-mysqlnd php7.1-gd php7.1-fpm php7.1-mbstring php7.1-xml unzip nginx mysql-server php7.1-xdebug

info "Configure MySQL"
sed -i "s/.*bind-address.*/bind-address = 0.0.0.0/" /etc/mysql/mysql.conf.d/mysqld.cnf
mysql -uroot <<< "CREATE USER 'root'@'%' IDENTIFIED WITH mysql_native_password BY 'root'"
...
mysql --user=root --password=root <<< "CREATE DATABASE shop2"
mysql --user=root --password=root <<< "CREATE DATABASE shop2_test"
echo "Done!"
